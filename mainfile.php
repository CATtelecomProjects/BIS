...
$htmltags .= "ไม่อนุญาตให้ใช้งานคำสั่ง html tags ที่คุณระบุ!!!</b><br><br>";
$htmltags .= "[ <a href=\"javascript:history.go(-1)\"><b>กลับไป</b></a> ]</center>";
...
if(!$dbname) {
    die("<br><br><center><img src=images/logo.gif><br><br><b>คุณยังไม่ได้ติดตั้ง PHP-Nuke <br>(ค่าติดตั้งในไฟล์ config.php ยังเป็นค่าเดิมอยู่)<br><br>คุณสามารถติดตั้งได้โดย <a href='./install/index.php'>คลิกที่นี่</a> </center></b>");
}
...
$domain = str_replace("https://", "", $nukeurl);
$mtime = microtime();
$mtime = explode(" ",$mtime);
...
function delQuotes($string){
	...
	while ($i<$len) {
		...
		if ($quote==0) {
			...
			$quote=0;
			if (($attrib>0) && (isset($tmp))) { $result .= "=\"$tmp\""; }
			$tmp="";
			$attrib=-1;
		}
		break;
...
function formatTimestamp($time) {
	...
	//setlocale(LC_TIME, 'th_TH.TIS-620');
    if (!is_numeric($time)) {
