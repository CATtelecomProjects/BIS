...
/* version 3.0                                              */
/* Theme inspired by the phpbb2 style Orange by             */
...
include("themes/Orange/tables.php");
...
function themeheader() {
    global  $admin, $user, $banners, $sitename, $slogan, $cookie, $prefix, $db, $nukeurl, $anonymous, $name, $Version_Num;
	if ($Version_Num > "7.7") {
		$showbanners = ads(0);
	} else {   
	if ($banners == 1) {    
...
/* Check if this impression is the last one and print the banner */ 
   ...
   if (($imptotal <= $impmade) AND ($imptotal != 0)) { 
    ...
    echo "<body bgcolor=\"#FFCD01\" text=\"#000000\" leftmargin=\"20\" topmargin=\"20\" marginwidth=\"20\" marginheight=\"20\">";

    if ($username == "Anonymous") {
	$theuser = "&nbsp;&nbsp;<a href=\"modules.php?name=Your_Account\">"._LOGIN."</a> หรือ <a href=\"modules.php?name=Your_Account&op=new_user\">"._BREG."</a>";
    } else {
...
function themefooter() {
	...
	$tmpl_file = "themes/Orange/center_right.html";
	$thefile = implode("", file($tmpl_file));
	$thefile = addslashes($thefile);
	$thefile = "\$r_file=\"".$thefile."\";";
	eval($thefile);
    ...
    $tmpl_file = "themes/Orange/footer.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
// YOU CAN ONLY ADD TO IT IF YOU MODIFY THIS THEME :-)
        echo "<center><br><font class=\"small\">:: Orange phpbb2 style by <a href=\"http://www.wabisurf.net/\" target=\"_blank\">Wabisurf</a> :: PHP-Nuke theme by <a href=\"http://www.nukemods.com\" target=\"_blank\">www.nukemods.com</a> :: Thai Edition by <a href=\"http://www.thainuke.org\">ThaiNuke.org</a> :: </font></center>";  
	echo "<br>\n";
}
...
function themeindex ($aid, $informant, $time, $title, $counter, $topic, $thetext, $notes, $morelink, $topicname, $topicimage, $topictext) {
    ...
    $tmpl_file = "themes/Orange/story_home.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themearticle ($aid, $informant, $datetime, $title, $thetext, $topic, $topicname, $topicimage, $topictext) {
    ...
    $tmpl_file = "themes/Orange/story_page.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themesidebox($title, $content) {
    $tmpl_file = "themes/Orange/blocks.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
