...
/************************************************************/
/* Theme Name: phpBB2Plus                           */
/* Theme Design: coldblooded (www.nukemods.com)      */
/*							   : thainuke (www.thainuke.org)             */
/* version 1.0                                              */
/* Theme inspired by the phpbb2 style FI Theme by      */
...
include("themes/phpBB2Plus/tables.php");
...
function themeheader() {
    global  $admin, $user, $banners, $sitename, $slogan, $cookie, $prefix, $db, $nukeurl, $anonymous, $name, $Version_Num, $lang;
	if ($Version_Num > "7.7") {
		$showbanners = ads(0);
	} else {   
	if ($banners == 1) {    
...
/* Check if this impression is the last one and print the banner */ 
   ...
   if (($imptotal <= $impmade) AND ($imptotal != 0)) { 
    ...
    $tmpl_file = "themes/phpBB2Plus/header.html";
   ...
   */
    $tmpl_file = "themes/phpBB2Plus/left_center.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themefooter() {
	...
	$tmpl_file = "themes/phpBB2Plus/center_right.html";
	$thefile = implode("", file($tmpl_file));
	$thefile = addslashes($thefile);
	$thefile = "\$r_file=\"".$thefile."\";";
	eval($thefile);
...
/*echo"<table width=\"100%\" border=\"0\" cellpadding=\"4\">"
  . "<tr>"
  . "<td><div align=\"center\"><font class=\"small\">phpBB2Plus Theme by <a href=\"http://www.forumimages.com/\">FI Theme</a> + <a href=\"http://www.nukemods.com\">nukemods.com</a> + <a href=\"http://www.thainuke.org\">ThaiNuke</a></font></div></td>"
  . "</tr>"
  . "</table>"
 ."";
 */
...
function themeindex ($aid, $informant, $time, $title, $counter, $topic, $thetext, $notes, $morelink, $topicname, $topicimage, $topictext) {
   global $anonymous, $tipath;
    ...
    $tmpl_file = "themes/phpBB2Plus/story_home.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themearticle ($aid, $informant, $datetime, $title, $thetext, $topic, $topicname, $topicimage, $topictext) {
    ...
    $tmpl_file = "themes/phpBB2Plus/story_page.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themesidebox($title, $content) {
    $tmpl_file = "themes/phpBB2Plus/blocks.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
