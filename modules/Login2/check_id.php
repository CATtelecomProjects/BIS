<?
	...
	if ($plevel != "")	{
	 $sqlCount = "SELECT *
  							   FROM USER_LOGIN
 							   WHERE 
							   				USERNAME  = '".$_GET['username']."'  AND PASSWORD = '".base64_encode($_GET['password'])."' 
...
/*if (file_exists("./includes/custom_files/dbConnect.php")) {
		include("./includes/custom_files/dbConnect.php");
			...
			#=================================================================================#
			 ...
			 $sqlLastLogin = "UPDATE user_login SET LAST_LOGIN = TO_DATE('".date('Y/m/d h:i:s A')."' ,'yyyy/mm/dd:hh:mi:ssam')  WHERE USERNAME = '".$_GET['username']."'";
		...
		echo "<center>";
		echo "<br><span style=\"color:green;font-weight:bold\">ยินดีต้อนรับ $_SESSION[desc] เข้าสู่ระบบ</span><br>&nbsp;<br>";	
		echo "<font size='9px'>เข้าใช้งานล่าสุด ".datethaiTime($_SESSION['lastlogin'])."</font><br>&nbsp;<br>";	
	    echo "<input type=\"button\" name=\"button\" id=\"button\" value=\"เข้าสู่หน้าหลัก\" onclick=\"window.location = 'http://bis.cattelecom.com'\" style=\"cursor:pointer\" />";
		echo "</center>";
		?>
			<!--
	...
	echo "<input type=\"button\" name=\"button\" id=\"button\" value=\"เปลี่ยนรหัสผ่านใหม่\" onclick=\"displayMessage('modules.php?name=Login&op=chgpass');return false\" style=\"cursor:pointer\" />";
		?>
		<!--
        	<script language = "javascript">
	...
	echo "<input type=\"button\" name=\"button\" id=\"button\" value=\"เข้าสู่ระบบใหม่\" onclick=\"displayMessage('./modules/Login2/dialog_Login.php');return false\" style=\"cursor:pointer\" /><br>&nbsp;<br>";
		?>
		<!--	<script language = "javascript">
