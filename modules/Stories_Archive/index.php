...
function show_month($year, $month, $month_l) {
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		if ($catid == 0) {
			...
			$title = "<a href=\"modules.php?name=News&amp;file=categories&amp;op=newindex&amp;catid=$catid\">$cat_title</a>: <a href=\"modules.php?name=News&amp;file=article&amp;sid=$sid$r_options\">$title</a>";
		}
		if ($multilingual == 1) {
			if (empty($alanguage)) {
				$alanguage = $language;
