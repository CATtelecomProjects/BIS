...
/*****************************************************************/		
		function user_login_db(){		
			...
			/*$str_type = $this->user_type == "1" ? " AND PASSWORD = '".base64_encode($this->password)."' " : " AND PASSWORD = '".base64_encode($this->password)."' ";
			 ...
			 $sqlLogin = "SELECT USERNAME,
							  ...
							  FROM USER_LOGIN
							 WHERE 
										USERNAME = '".$this->username."'  $str_type 
										" ;
			*/
		...
		function update_stat(){
			...
			$sqlStat = "INSERT INTO count_log (USERNAME,EMP_ID,ORG_CODE,LOG_DATE,IP,LOGOUT_TIME,SESSION_ID,IP_REAL) VALUES ('$this->username','".$this->rs_data['EMP_ID']."','".$this->rs_data['ORG_CODE']."',CURRENT_TIMESTAMP,'".$_SERVER['REMOTE_ADDR']."', null,'".$this->randText."','".$_SERVER['HTTP_X_FORWARDED_FOR']."')";
			...
			*/
			...
			#=================================================================================#
			 ...
			 $sqlLastLogin = "UPDATE user_login SET LAST_LOGIN = CURRENT_TIMESTAMP  WHERE USERNAME = '".$this->username."'";
...
########################################################
	...
	function check_expire(){
		  $sqlCount = "SELECT USERNAME
  							   FROM USER_LOGIN
 							   WHERE 
							   				USERNAME  = '".$this->username."'  	AND EXPIRE_DATE >= to_date('$this->dates','yyyy/mm/dd')";	
	...
	require('../../simplesamlphp/www/saml_include.php');
	...
	$Auth->username = $attributes["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"][0];
	$Auth->username = substr($Auth->username, 0, stripos($Auth->username, "@"));
