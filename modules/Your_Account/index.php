...
function confirmNewUser($username, $user_email, $user_password, $user_password2, $random_num, $gfx_check) {
	...
	if (!$stop) {
		...
		."<tr><td><b>"._UPASSWORD.":</b> $user_password<br></td></tr>"
		."<tr><td><b>"._EMAIL.":</b> $user_email</td></tr></table><br><br>";
		if ($autoActivate) echo "<center><b>"._NOTE."</b>"._YA_NOTE."";
...
function activate($username, $check_num, $autoActivate=FALSE) {
	...
	if ($db->sql_numrows($result) == 1) {
		...
		if ($check_num == $row['check_num']) {
			...
			if (!$autoActivate) include("header.php"); // TN
...
function userinfo($username, $bypass=0, $hid=0, $url=0) {
	...
	if(($num == 1) && ($userinfo['user_website'] || $userinfo['femail'] || $userinfo['bio'] || $userinfo['user_avatar'] || $userinfo['user_icq'] || $userinfo['user_aim'] || $userinfo['user_yim'] || $userinfo['user_msnm'] || $userinfo['user_location'] || $userinfo['user_occ'] || $userinfo['user_interests'] || $userinfo['user_sig'])) {
		...
		if (is_admin($admin)) {
			...
			OpenTable2();
			echo "<center><b>"._CHANGEKARMA." ".$userinfo['username']."</b><br><br>";
...
function edituser() {
	...
	if ((is_user($user)) AND (strtolower($userinfo['username']) == strtolower($cookie[1])) AND ($userinfo['user_password'] == $cookie[2])) {
	   ...
	   // Avatar control panel
		...
		."<BR><b>"._YA_AVCP."</b>"
		."<tr><td bgcolor='$bgcolor2'>"._YA_AVINF1."&nbsp;".$board_config['avatar_max_width']."&nbsp;"._YA_AVINF2."&nbsp;".$board_config['avatar_max_height']."&nbsp;"._YA_AVINF3."&nbsp;".CoolSize($board_config['avatar_filesize']).".</td>";
		echo "<td bgcolor='$bgcolor3' align=center>"._YA_CURRAV."<BR><BR><IMG alt=\"\" src=\"$userinfo[user_avatar]\"></td></tr><BR>";
		if ($board_config['allow_avatar_local']) {
			echo "<form action=\"modules.php?name=Your_Account&amp;op=avatarlist\" method=\"post\">"
			."<tr><td bgcolor='$bgcolor2'><b>"._YA_SELAVGALL.":</b></td>"
			."<td bgcolor='$bgcolor3'>".$s_categories."&nbsp;<img src=\"images/right.gif\" align=middle>&nbsp;<INPUT class=button type=submit value=\""._YA_SHOWGALL."\"></td></tr>"
			."</form>";
		} else {
			echo "<tr><td bgcolor='$bgcolor2'><b>"._YA_SELAVGALL.":</b></td>"
			."<td bgcolor='$bgcolor3'><b>"._YA_DISABLED."</b></td></tr>";
		}
		if ($board_config['allow_avatar_upload']) {
			echo "<tr><td bgcolor='$bgcolor2'><b>"._YA_UPLOADAV.":</b></td>"
			."<td bgcolor='$bgcolor3'><a href=\"modules.php?name=Forums&amp;file=profile&amp;mode=editprofile\"><b>"._YA_UPLOADFORUM."</b></a></td></tr>"
			."<tr><td bgcolor='$bgcolor2'><b>"._YA_UPLOADURL.":</b><br><SPAN class=gensmall>"._YA_AVCOPIED."</SPAN></td>"
			."<td bgcolor='$bgcolor3'><a href=\"modules.php?name=Forums&amp;file=profile&amp;mode=editprofile\"><b>"._YA_UPLOADFORUM."</b></a></td></tr>";
		} else {
			echo "<tr><td bgcolor='$bgcolor2'><b>"._YA_UPLOADAV.":</b></td>"
			."<td bgcolor='$bgcolor3'><b>"._YA_DISABLED."</b></td></tr>"
		...
		if ($board_config['allow_avatar_remote']) {
			...
			."<tr><td bgcolor='$bgcolor2'><b>"._YA_OFFSITE.":</b><br><SPAN class=gensmall>"._YA_SUBMITBUTTON."</SPAN></td>"
		...
		echo "<tr><td bgcolor='$bgcolor3' colspan='2' align='center'>"
		."<INPUT class=mainoption type=submit value="._YA_SUBMIT.">"
		."</form></TD></TR></TABLE>";
		CloseTable();
		include("footer.php");
	} else {
...
function avatarsave($avatar, $category) { 
   ...
   if (is_user($user) AND $allow_avatar_local) { 
		...
		echo "<center><font class=\"content\">"._YA_AVATARFOR."".$cookie[1]." "._YA_SAVED."</center></font><br><br>";
		if (ereg("(http)", $newavatar)) { echo "<center>"._YA_NEWAVATAR.":<br><br><IMG alt=\"\" src=\"$newavatar\"><br><br> [ <a href=\"modules.php?name=$module_name&amp;op=edituser\">"._YA_BACKPROFILE."</a> | <a href=\"modules.php?name=$module_name\">"._YA_DONE."</a> ]<br><br></center>"; } elseif ($newavatar) { echo "<center>"._YA_NEWAVATAR.":<br><br><IMG alt=\"\" src=\"modules/Forums/images/avatars/$newavatar\"><br><br>[ <a href=\"modules.php?name=$module_name&amp;op=edituser\">"._YA_BACKPROFILE."</a> | <a href=\"modules.php?name=$module_name\">"._YA_DONE."</a> ]<br><br></center>"; }
		} else { 
		   echo "<center>"._YA_ERROR2."<br />"._GOBACK."</center>"; 
		} 
		CloseTable();
		include("footer.php");
	}
...
function avatarlinksave($avatar) {
	...
	if (is_user($user) AND $allow_avatar_remote) { 
		...
		echo "<center><font class=\"content\">"._YA_AVATARFOR."".$cookie[1]." "._YA_SAVED."</center></font><br><br>";
		if (ereg("(http)", $avatar)) { echo "<center>"._YA_NEWAVATAR.":<br><br><IMG alt=\"\" src=\"$avatar\"><br><br>[ <a href=\"modules.php?name=$module_name&amp;op=edituser\">"._YA_BACKPROFILE."</a> | <a href=\"modules.php?name=$module_name\">"._YA_DONE."</a> ]<br><br></center>"; } elseif ($avatar) { echo "<center>"._YA_NEWAVATAR.":<br><br><IMG alt=\"\" src=\"modules/Forums/images/avatars/$avatar\"><br><br>[ <a href=\"modules.php?name=$module_name&amp;op=edituser\">"._YA_BACKPROFILE."</a> | <a href=\"modules.php?name=$module_name\">"._YA_DONE."</a> ]<br><br></center>"; }
		} else { 
		  echo "<center>"._YA_ERROR2."<br />"._GOBACK."</center>"; 
		} 
		CloseTable();
