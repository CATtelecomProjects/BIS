...
//--------  Select เอาข้อมูลที่มีมาใส่ใน  ตัวแปร JavaScript ก่อน
		...
		$sql = $sql." where emp_id = '".$emp_id."' and topic_id = '3.5'";
						...
						if ( !Isdouble(frm.percent_sucess.value) )
						...
						else
							{
								if ( !Isdouble(frm.score_level.value) )
									{
										alert("กรุณาป้อนข้อมูล ' ผลประเมินปลายปี ที่คาดว่าจะทำได้รับ ' เป็นตัวเลขค่ะ...");
...
// ------------------------------------   END  Dropdown list   ปี พ.ศ.    --------------------------------------
	...
	//  ++++  Check ก่อนว่ามีข้อมูลตามปีที่ แสดง ใน Table tris_topic_group3 หรือไม่  ถ้าไม่มีก็แสดงว่า ไม่มี ข้อมูลเลย  ++++
		...
		if ($chkrows == 0) // check ว่า   ถ้าไม่มีข้อมูล ใน Table Master   tris_topic   ของปีที่แสดง  ให้ขึ้นว่าไม่มีข้อมูล
			...
			{ 
	            echo '<form name="frm" action = "save3_5_save.php" method = "post">';
				...
				echo "<font size='4' color='#0099ff'>บันทึกข้อมูลการบริหารจัดการทรัพยากรบุคคล</font>";
				echo "<br><br>";
				...
				//====  Hidden field ของ รหัส topic_id
					echo '<INPUT TYPE="hidden" NAME="stopicid" value="3.5">';
				//====
...
//-----------------------------------------------  START  ผลประเมินปลายปี ที่คาดว่าจะทำได้รับ  -----------------------------------------------------
								echo "<tr>";
										echo "<td align='left' width=".$tdwidth_td1.">";
										echo '<font size="3" face="MS Sans Serif">ผลประเมินปลายปี ที่คาดว่าจะทำได้รับ</font>';
										echo "</td>";

										echo "<td align='left' width=".$tdwidth_td2.">";
										echo '<input type="text" name="score_level" maxlength="5" size="5"  tabindex="9" onKeyDown=chk_enter()>';
										echo "</td>";

								echo "</tr>";
				//-----------------------------------------------  END  ผลประเมินปลายปี ที่คาดว่าจะทำได้รับ  -----------------------------------------------------
