																...
																switch($year_predict)
																	{
																		...
																		case ( ($year_predict > $c1) &&  ($year_predict < $c2)  ) :
																					$yr_predict = ( ($c2 - $year_predict) / ($c2 - $c1) ) + 1 ;
																			break;

																		// อยู่ระหว่าง  ระดับ 2  และ  ระดับ   3
																		case ( ($year_predict > $c2) &&  ($year_predict < $c3)  ) :
																					$yr_predict = ( ($c3 - $year_predict) / ($c3 - $c2) ) + 2 ;
																			break;

																		// อยู่ระหว่าง  ระดับ 3  และ  ระดับ   4
																		case ( ($year_predict > $c3) &&  ($year_predict <= $c4)  ) :
																					$yr_predict = ( ($c4 - $year_predict) / ($c4 - $c3) ) + 3 ;
																			break;

																		// อยู่ระหว่าง  ระดับ 4  และ  ระดับ   5
																		case ( ($year_predict > $c4) &&  ($year_predict < $c5)  ) :
																					$yr_predict = ( ($c5 - $year_predict) / ($c5 - $c4) ) + 4 ;
																			break;
																			 // เท่ากับ ระดับที่กำหนด
																		case  ($year_predict = $c1) : $yr_predict = 1; break;
																		case  ($year_predict = $c2) : $yr_predict = 2; break;
																		case  ($year_predict = $c3) : $yr_predict = 3; break;
																		case  ($year_predict = $c4) : $yr_predict = 4; break;
																		case  ($year_predict = $c5) : $yr_predict = 5; break;
									...
									switch ($criteriatype)
										{
											case "1" :  //  criteria_type = '1' ให้เก็บลง  tris_operation_quality_tot
												...
												break;
											case "2" :   //   criteria_type = '2' ให้เก็บลง tris_operation_volume
																	$sql_ins_data = " insert into tris_operation_volume(year , month , topic_id , year_predict , emp_id )  ";
