					...
					while (ocifetch($parse_sql))
						{
							...
							$subtopicid = str_replace(".","_",$subtopic_id);
							echo $emp_id."_".$yyr."_".$mmn."Z".$subtopicid." = '".$operation_goal."|".$operation_level."|".chk_feedline($reason)."|".chk_feedline($solution)."|".$active_flag."|".chk_feedline($progress)."'; ";
						}
			}
	...
	//  ++++  Check ก่อนว่ามีข้อมูลตามปีที่ แสดง ใน Table tris_topic_group3 หรือไม่  ถ้าไม่มีก็แสดงว่า ไม่มี ข้อมูลเลย  ++++
		...
		if ($chkrows == 0) // check ว่า   ถ้าไม่มีข้อมูล ใน Table Master   tris_topic   ของปีที่แสดง  ให้ขึ้นว่าไม่มีข้อมูล
			...
			{ 
				...
				echo "<font size='4' color='#0099ff'>บันทึกข้อมูลการบริหารความเสี่ยง</font>";
				echo "<br><br>";
				...
				//====  Hidden field ของ รหัส topic_id
					echo '<INPUT TYPE="hidden" NAME="stopicid" value="3.2">';
				//====
...
/*   ซ่อน ไว้ก่อน  โดยทำเป็น hidden
								...
								echo "</tr>";
*/
								echo '<INPUT TYPE="hidden" id="y_active_flag"   NAME="active_flag" VALUE = "1">';
								echo '<INPUT TYPE="hidden" id="n_active_flag"   NAME="active_flag" VALUE = "0">';
...
/*   ซ่อน ไว้ก่อน  โดยทำเป็น hidden
								echo "<tr>";
										...
										echo "<td align='left' width=".$tdwidth_td2.">";
										echo '<textarea name="reason" rows="5" cols="50"  tabindex="7" onKeyDown=chk_enter()></textarea>';
										echo "</td>";
								echo "</tr>";
*/
