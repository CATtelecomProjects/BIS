...
//--------  Select เอาข้อมูลที่มีมาใส่ใน  ตัวแปร JavaScript ก่อน
		...
		$sql = "select year, substr(subtopic_id,1,1) || '.' || substr(subtopic_id,3,4)  ";
		$sql = $sql." from tris_topic_group3  ";
		$sql = $sql." where topic_id = '3.3.2'   ";
		$sql = $sql."  and subtopic_id like '2%'  ";
		$sql = $sql." order by year , to_number(substr(subtopic_id,1,1)) , to_number(substr(subtopic_id,3,4))  ";
		...
		$sql = $sql." where emp_id = '".$emp_id."'  and topic_id ='3.3.2' ";
		$sql = $sql." and subtopic_id like '2%' ";
		$sql = $sql." order by emp_id , year , month , topic_id , subtopic_id ";
...
<?  include("./includes/head3-3-2.php"); ?>
...
// ------------------------------------   END  Dropdown list   ปี พ.ศ.    --------------------------------------
	...
	//  ++++  Check ก่อนว่ามีข้อมูลตามปีที่ แสดง ใน Table tris_topic_group3 หรือไม่  ถ้าไม่มีก็แสดงว่า ไม่มี ข้อมูลเลย  ++++
		...
		if ($chkrows == 0) // check ว่า   ถ้าไม่มีข้อมูล ใน Table Master   tris_topic   ของปีที่แสดง  ให้ขึ้นว่าไม่มีข้อมูล
			...
			{ 
	            echo '<form name="frm" action = "save3_3_2_save.php" method = "post">';
				...
				echo "<font size='4' color='#0099ff'>บันทึกข้อมูลการตรวจสอบภายใน</font>";
				echo "<br><br>";
				...
				//====  Hidden field ของ รหัส topic_id
					echo '<INPUT TYPE="hidden" NAME="stopicid" value="3.3.2">';
				//====
