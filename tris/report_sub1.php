...
//echo "mn=".$mn."<br>yr=".$yr."<br>topicid=".$topicid."<br>tb=".$tb."<br>ff=".$ff."<br>criteriatype=".$criteriatype;
	...
	$nametopic = "...";
	$col_year = 2 ; //  กำหนดจำนวน column ของ ปี ว่าจะให้แสดงกี่ปี

	if (!empty($topicid))  // select เอาค่าของ ชื่อ topic
	{
				$sql = " select topic_name from tris_topic where topic_id = '".$topicid."' and year = '".($yr+1)."' ";
				...
				while (ocifetch($data_sql))
					{
						...
						{
							$$var_yr = number_format($val_num,2,".",",");
						}
									...
									for ($y=($yr-($col_year-1)) ; $y<=($yr) ; $y++) 
										{
											...
											echo '<td align="center" style="border-width:1; border-color:black; border-style:solid;"><font size="2" face="MS Sans Serif">'.$$var_yr.'</font></td>';
										}
