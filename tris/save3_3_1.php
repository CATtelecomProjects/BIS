...
//--------  Select เอาข้อมูลที่มีมาใส่ใน  ตัวแปร JavaScript ก่อน
		...
		//+++++++ start +++++++++   ข้อมูล  topic_id  ของแต่ล่ะปี   +++++++++++++++++++++++++++++++++++++++++++
			...
			$sql = $sql." where topic_id = '3.3.1' ";
			$sql = $sql." and subtopic_id  not like '6%' ";
					...
					while (ocifetch($parse_sql))
						{
							...
							$add_progress = ociresult($parse_sql,6);
							$add_solution = ociresult($parse_sql,7);

							$subtopicid = str_replace(".","_",$subtopic_id);

							echo " T_".$yrr."Z".$subtopicid." = '" .chk_feedline($subtopic_name)."|".$goal."|" .chk_feedline($add_progress)."|".chk_feedline($add_solution).  "';  ";
						}
			}
		...
		$sql = $sql." where emp_id = '".$emp_id."' and topic_id = '3.3.1'   ";
		$sql = $sql." and subtopic_id  not like '6%' ";
			...
			function show_g1detail()
				{
					...
					//---- Start แสดงข้อมูล เกณฑ์วัดผลการดำเนินงาน
							...
							add_pro = tmp_g1_val_split[2];
							add_sol = tmp_g1_val_split[3];
...
<?  include("./includes/head3-3-1.php"); ?>
...
// ------------------------------------   END  Dropdown list   ปี พ.ศ.    --------------------------------------
	...
	//  ++++  Check ก่อนว่ามีข้อมูลตามปีที่ แสดง ใน Table tris_topic_group3 หรือไม่  ถ้าไม่มีก็แสดงว่า ไม่มี ข้อมูลเลย  ++++
		...
		if ($chkrows == 0) // check ว่า   ถ้าไม่มีข้อมูล ใน Table Master   tris_topic   ของปีที่แสดง  ให้ขึ้นว่าไม่มีข้อมูล
			...
			{ 
	            echo '<form name="frm" action = "save3_3_1_save.php" method = "post">';
				...
				echo "<font size='4' color='#0099ff'>บันทึกข้อมูลการควบคุมภายใน</font>";
				echo "<br><br>";
				...
				//====  Hidden field ของ รหัส topic_id
					echo '<INPUT TYPE="hidden" NAME="stopicid" value="3.3.1">';
				//====
...
*/
	...
	//-----------------------------------------------  START  ความคืบหน้าการดำเนินการจากปีที่ผ่านมา  ---------------------------
								echo "<tr>";
										...
										echo "<td align='left' width=".$tdwidth_td2.">";
										echo '<textarea name="progress" rows="5" cols="50"   onKeyDown=chk_enter()></textarea>';
										echo "</td>";
				...
				//-----------------------------------------------  START  เสนอแนวทางแก้ไข  -----------------------------------------------------
								echo "<tr>";
										...
										echo "<td align='left' width=".$tdwidth_td2.">";
										echo '<textarea name="solution" rows="5" cols="50"    onKeyDown=chk_enter()></textarea>';
										echo "</td>";
				...
				//-----------------------------------------------  START  ผลประเมินปลายปี ที่คาดว่าจะทำได้  -----------------------------------------------------
								echo "<tr>";
										...
										echo "<td align='left' width=".$tdwidth_td2.">";
										echo '<input type="text" name="score_level" maxlength="6" size="5"   onKeyDown=chk_enter()>';
										echo "</td>";
				...
				//-----------------------------------------------  START  ปุ่ม SAVE  -----------------------------------------------------
								echo "<tr>";
										...
										echo '<input type="button" name="save" value="บันทึก"  onclick=send_save()>';
										echo "&nbsp;";
										echo '<input type="button" name="cancle" value="ยกเลิก"   onclick=cleardata()>';
										echo "</td>";
								echo "</tr>";
				//-----------------------------------------------  END  ปุ่ม SAVE  -----------------------------------------------------
